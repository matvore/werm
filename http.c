...
static int b64ops(Httpreq *rq, struct wrides *errout)
{
	...
	unsigned binhashl, outlen = sizeof(acceptwskey);
	char sslerrbuf[256], *out = acceptwskey;
	long sslerr;
	...
	if (	!BIO_write_ex(bmem, reqcr, strlen(reqcr), &writn)
	||	writn != strlen(reqcr)
	||	!BIO_write_ex(bmem, "\n", 1, &writn)
	|| 	writn != 1
	||	!BIO_read_ex(b64, challkey, sizeof(challkey), &redn))
		goto dumperr;
	...
	/* Switch to EVP_MD_fetch if EVP_sha1 is called multiple times in a
	   process. */
	if (	!EVP_DigestInit_ex(s1ctx, EVP_sha1(), 0)
	||	!EVP_DigestUpdate(s1ctx, reqcr, strlen(reqcr)))
		goto dumperr;
	...
	if (!BIO_read_ex(bmem, out, outlen, &redn))	goto dumperr;
	if (redn != outlen)				goto dumperr;
...
void http_read_req(FILE *src, Httpreq *rq, struct wrides *respout)
{
	...
	for (;;) {
		...
		if (consumereqln("sec-websocket-key:")) {
			if (!b64ops(rq, respout)) goto seterr;
			continue;
		}
