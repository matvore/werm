...
function newcred()
{
	...
	crarg = {
		challenge: chal,
		rp: {name: window.relyingparty, id: window.relyingparty},
		user: {
			id: new Uint8Array(window.wermuserid),
			name: window.wermauthnuser,
			displayName: window.wermauthnuser,
		},
		pubKeyCredParams: [{alg: FIDOALGTYPE, type: 'public-key'}],
...
function doauthn()
{
	...
	if (athreq.status != 200) {
		termwrite('failure checking authn status\n');
		console.log('XMLHttpRequest failure:', athreq);
		return;
	}
	...
	prom.then(function (asr)
	{
		...
		console.log('authn assertion:', asr);
		...
		{
			if (r.status == 200) { window.onload(); return }
